...
// Built by tools/build_dev_bundle.py as dev_army_bundle.js.
...
/* ===== BEGIN classic_undead_champion_fix.js ===== */
...
;
/* ===== END classic_undead_champion_fix.js ===== */
...
(() => {
  ...
  function clearMagicDuplicates(entry) {
    const removed = Boolean(entry.magicBanner || (entry.magicItems || []).length || (entry.champion?.magicItems || []).length);
  ...
  function refreshMobileRosterScrollMetrics() {
    if (!window.matchMedia?.("(max-width: 900px)").matches) return;
    const panel = els.roster?.closest?.(".roster-panel");
    if (!panel) return;
  ...
  function duplicateRegiment(entryId) {
    ...
    renderArmy();
    refreshMobileRosterScrollMetrics();
    showToast(omittedUniqueMagic ? "Regiment copied; unique magic items/banner were not duplicated" : "Regiment copied");
  }
  ...
  function reorderWithinSection(draggedId, targetId, before) {
    ...
    const ordered = state.roster.filter(entry => entry.sectionKey === dragged.sectionKey && entry.id !== draggedId);
    const targetIndex = ordered.findIndex(entry => entry.id === targetId);
    if (targetIndex < 0) return;
    ordered.splice(targetIndex + (before ? 0 : 1), 0, dragged);
    let cursor = 0;
    state.roster = state.roster.map(entry => entry.sectionKey === dragged.sectionKey ? ordered[cursor++] : entry);
    renderArmy();
    refreshMobileRosterScrollMetrics();
  ...
  function clearDropMarkers() {
    els.roster.querySelectorAll(".roster-card-drop-before, .roster-card-drop-after, .roster-card-dragging").forEach(card => card.classList.remove("roster-card-drop-before", "roster-card-drop-after", "roster-card-dragging"));
  }

  function markDropTarget(card, clientY) {
    els.roster.querySelectorAll(".roster-card-drop-before, .roster-card-drop-after").forEach(other => other.classList.remove("roster-card-drop-before", "roster-card-drop-after"));
    if (!card) return null;
  ...
  function wireDesktopDrag(card, handle, entry) {
    ...
    handle.addEventListener("dragend", () => { dragState = null; clearDropMarkers(); });
  ...
  function wirePointerDrag(card, handle, entry) {
    handle.addEventListener("pointerdown", event => {
      ...
      event.preventDefault();
      touchState = { id: entry.id, sectionKey: entry.sectionKey, pointerId: event.pointerId, targetId: null, before: true };
  ...
  function decorateRegimentCards() {
    ...
    const regimentSection = [...els.roster.querySelectorAll(".roster-section")].find(section => section.querySelector(".roster-section-title")?.textContent.trim() === "Regiments");
...
/* ===== BEGIN chaos_daemon_regiment_items.js ===== */
...
;
/* ===== END chaos_daemon_regiment_items.js ===== */

/* ===== BEGIN dogs_of_war_regiments_of_renown.js ===== */
...
;
/* ===== END dogs_of_war_regiments_of_renown.js ===== */

/* ===== BEGIN general_system.js ===== */
...
;
/* ===== END general_system.js ===== */

/* ===== BEGIN general_overrides.js ===== */
...
;
/* ===== END general_overrides.js ===== */

/* ===== BEGIN dev_roster_pad_sort.js ===== */
