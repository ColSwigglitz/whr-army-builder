...
values (
  'phoenix_games',
  'Phoenix Games',
  'The initial WHR Army Builder campaign format. Additional campaign formats can be added later.',
  true
)
...
create unique index if not exists campaign_applications_one_pending
  on public.campaign_applications(campaign_id, applicant_id)
...
create unique index if not exists campaign_invites_one_pending
  on public.campaign_invites(campaign_id, invited_user_id)
  where status = 'pending';
...
-- Automatically add the creator as the campaign owner/member.
...
create trigger whr_campaign_owner_member_trigger
after insert on public.campaigns
for each row execute function public.whr_campaign_add_owner_member();
...
-- Campaign types are safe reference data for signed-in users.
drop policy if exists "Campaign types readable by signed in users" on public.campaign_types;
create policy "Campaign types readable by signed in users"
  on public.campaign_types for select to authenticated using (active = true);

-- Campaigns: public campaigns are discoverable. Private campaigns are visible to
-- their owner, members and users with a pending invite.
...
create policy "Users can create owned campaigns"
...
-- Membership is readable for campaigns the user can access. Membership writes
-- are performed through the security-definer workflow functions below.
...
-- Applications: applicants can see/create their own application; campaign owners
-- can see applications made to campaigns they own.
...
create policy "Applicants and owners can read campaign applications"
  ...
  using (
    ...
    or exists (
      select 1 from public.campaigns c
      where c.id = campaign_applications.campaign_id and c.owner_id = auth.uid()
    )
  );
...
create policy "Users can apply to public campaigns"
  ...
  with check (
    ...
    and not exists (
      select 1 from public.campaign_members cm
      where cm.campaign_id = campaign_applications.campaign_id and cm.user_id = auth.uid()
    )
  );
...
-- Invites are visible to the invited user and campaign owner. Owners may create
-- and cancel invitations. Accept/decline uses the workflow RPC below.
...
create policy "Invitees and owners can read campaign invites"
  ...
  using (
    ...
    or exists (
      select 1 from public.campaigns c
      where c.id = campaign_invites.campaign_id and c.owner_id = auth.uid()
    )
  );
...
create policy "Owners can create campaign invites"
  ...
  with check (
    ...
    and not exists (
      select 1 from public.campaign_members cm
      where cm.campaign_id = campaign_invites.campaign_id and cm.user_id = invited_user_id
    )
  );
...
-- Owner decision workflow: accepting an application atomically creates membership.
...
as $$
declare
  v_application public.campaign_applications%rowtype;
  v_owner uuid;
begin
  ...
  if v_owner is distinct from auth.uid() then
    raise exception 'Only the campaign owner can decide this application';
  end if;
  ...
  if p_decision = 'accepted' then
    insert into public.campaign_members(campaign_id, user_id, role)
    values (v_application.campaign_id, v_application.applicant_id, 'member')
    on conflict (campaign_id, user_id) do nothing;

    update public.campaign_invites
    set status = 'cancelled', responded_at = now()
    where campaign_id = v_application.campaign_id
      and invited_user_id = v_application.applicant_id
      and status = 'pending';
  end if;
end;
$$;
...
-- Invite response workflow: accepting an invite atomically creates membership.
...
begin
  ...
  if p_response = 'accepted' then
    insert into public.campaign_members(campaign_id, user_id, role)
    values (v_invite.campaign_id, v_invite.invited_user_id, 'member')
    on conflict (campaign_id, user_id) do nothing;

    update public.campaign_applications
    set status = 'withdrawn', decided_at = now()
    where campaign_id = v_invite.campaign_id
      and applicant_id = v_invite.invited_user_id
      and status = 'pending';
  end if;
end;
$$;
