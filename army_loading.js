...
// Dwarf elite choices and the Goblin Hewer are 0-1 units.
(() => {
  const isDwarfArmy = () => state.data?.faction?.id === "dwarfs";
  const restrictedNames = new Set(["longbeards", "long beards", "hammerers", "ironbreakers", "rangers", "goblin hewer", "goblin hewer war machine"]);
  const normalise = value => String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
  const isRestricted = unit => isDwarfArmy() && unit && restrictedNames.has(normalise(unit.name));
  ...
  function existingCopy(unit, ignoreEntryId = null) {
    return state.roster.find(entry => {
      ...
      return existing && normalise(existing.name) === normalise(unit.name);
    }) || null;
  }
